...
Puppet::Type.type(:maven).provide(:mvn) do
  ...
  def create
    plugin_version = @resource[:pluginversion].nil? ? "2.6" : @resource[:pluginversion]
    ...
    options = options.nil? || options.empty? ? "" : options
    user = @resource[:user]
    ...
    begin
      Timeout::timeout(timeout) do
        output, status = run_command(command, user, group)
